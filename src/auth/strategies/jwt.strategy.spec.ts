...
describe('JwtStrategy', () => {
  ...
  function makeStrategy(opts: {
    ...
    const users: any = { existsAndActive: jest.fn().mockResolvedValue(opts.userActive) };
    const invalidated: any = { has: jest.fn().mockResolvedValue(opts.invalidatedHas) };
    return new JwtStrategy(config, users, invalidated);
  }

  const payload = { sub: 1, username: 'u', role: 'DEVELOPER', jti: 'jti-1' };
  ...
  it('returns the user-shaped payload on success', async () => {
    ...
    expect(result).toEqual({ id: 1, username: 'u', role: 'DEVELOPER', jti: 'jti-1' });
  });
});
