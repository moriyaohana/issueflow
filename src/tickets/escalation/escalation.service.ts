...
export class EscalationService {
  ...
  async runEscalation(): Promise<number> {
      ...
      for (const [from, group] of buckets.priorityBumps) {
        ...
        await ticketRepo
          ...
          .set({
            ...
            // Bulk UPDATE bypasses @VersionColumn; bump manually.
            version: () => 'version + 1',
          })
    ...
    // Audit writes run outside the ticket TX: AuditLog is append-only and
    // audit.record swallows its own errors, so failures must not roll back
    // the bulk UPDATE.
    await Promise.all(
    ...
    // Bulk UPDATE doesn't touch entity instances; mirror the writes onto the
    // in-memory references so callers see post-bump fields.
    ...
    for (const ticket of overdue) {
      ...
      if (idx < TICKET_PRIORITY_ORDER.length - 1) {
        ...
        auditRows.push({
          ...
          // HIGH→CRITICAL flips overdue in the same tick.
          isOverdueTo:
